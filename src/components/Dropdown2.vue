...
export default {
  ...
  methods: {
    ...
    initializeList() {
      if (this.selected) {
        this.selectedValues = this.selected.map((value) => {
          return this.options.find((option) => option.code === value);
        });
        this.notSelectedValues = this.options.filter(
          (item) => !this.selected.some((selectedItem) => selectedItem === item.code)
        );
      } else {
        this.selectedValues = [];
    ...
    updateList() {
      ...
      this.notSelectedValues = this.options.filter((value) =>
        this.draftList.notSelected.some((code) => value.code === code)
      );
